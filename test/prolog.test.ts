...
// function tpc<T>(str: string): T {
// 	return explodingCast<T>(...);
// }
...
test('LL(1) Prolog member test 1', () => {
	prologTest([
		['member(X, [X | _]).', PrologGlobalInfo.ClauseAdded],
		['member(X, [_ | T]) :- member(X, T).', PrologGlobalInfo.ClauseAdded],
		['?- member(1, [1, 2, 4]).', ['Satisfied']],
		['?- member(2, [1, 2, 4]).', ['Satisfied']],
		['?- member(3, [1, 2, 4]).', ['Not satisfied']],
		['?- member(4, [1, 2, 4]).', ['Satisfied']]
	]);
});
